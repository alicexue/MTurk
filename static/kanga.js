...
var drawTriviaDisplay = function(trialVariables) {
	...
	if (waitRect == null) {
		waitRect = new rect(waitRect, canvas.width * 0.02);
	} else {
		waitRect.removeRect();
	}
	waitRect.showRect(canvas.width/2, canvas.height/2 + canvas.height/5, WHITE, canvas.width * 0.2, fontSize);

	waitTime = expVariables[currTrialN]['C_Wait'];
	if (waitText == null) {
		waitText = new textBox(waitText, "WAIT " + waitTime.toString() + "s", fontSize, BLACK);
	} else {
		waitText.removeText();
	}
	waitText.setText("WAIT "+waitTime.toString());
	waitText.showText(0, canvas.height/2 + canvas.height/5 + fontSize - fontSize*0.1, fontSize);
	waitText.textObj.setAttribute("font-family","Gill Sans");

	if (knowRect == null) {
		knowRect = new rect(knowRect, canvas.width * 0.02);
	} else {
		knowRect.removeRect();
	}
	knowRect.showRect(canvas.width/2 + canvas.width/4, canvas.height/2 + canvas.height/8, WHITE, canvas.width * 0.2, fontSize);

	if (knowText == null) {
		knowText = new textBox(knowText, "KNOW", fontSize, BLACK);
	} else {
		knowText.removeText();
	}
	knowText.showText(0 + canvas.width/4, canvas.height/2 + canvas.height/8 + fontSize - fontSize*0.1, fontSize);
	knowText.textObj.setAttribute("font-family","Gill Sans");
}
		...
		if (specialKeys.indexOf(e.key) > -1 && currTrialN == allTrials.length - 1) { 
			...
			if (inTriviaDisplay && triviaDisplayResponseKeys.indexOf(e.key) > -1) {
				if (currTrialN < expVariables.length) {
					...
					allTrials[currTrialN].C_SWK_rt = t2 - t1;
				...
				allTrials[currTrialN].C_Worth_rt = t2 - t1;
				drawConfirmationDisplay();
			}
		}
	}
...
var drawWaitDisplay = function(waitTime) {
	removeTriviaDisplay();
	var questionText = document.getElementById('html_text'); 
	questionText.innerHTML = "...";
	...
	if (inWaitDisplay == false) {
		inWaitDisplay = true;
		waitTime = expVariables[currTrialN]['C_Wait']*1000;
		allTrials[currTrialN].C_WaitStart = performance.now();
		waitPeriodTimer = setTimeout( function() {
			drawAnswerDisplay();
		},waitTime);
	}
}
...
var drawRateAnswerDisplay = function() {
	var questionText = document.getElementById('html_text'); 
	questionText.innerHTML = "Was the answer worth the wait?";
	var fontSize = canvas.width * 0.03;
	questionText.style.paddingTop = (canvas.height/2 - canvas.height/4).toString() + "px";
	drawNumbers();
...
var drawConfirmationDisplay = function() {
	...
	} else if (allTrials[currTrialN].rsp == 't') {
		waitRect.setColor(color);
		drawFixation = false;
