...
var drawSceneToCanvas = function(addInfoToExpResults, trialN, canvasCtx) {
	trialVariables = expVariables[trialN];
	var position = "CENTER";
	...
	if (stimuli[trialN] == null) {
		stimuli[trialN] = [img];
	} else if (stimuli[trialN].length != nStimuliPerTrial) {
...
var currStim1;
var currStim2;
var drawChoiceStimuliToCanvas = function(addInfoToExpResults, trialN, canvasCtx) {
	trialVariables = expVariables[trialN];
...
var instructions;
var confirmationBox;
var drawSceneDisplay = function(addInfoToExpResults) {
	...
	if (trialCanvas.width == canvas.width && trialCanvas.height == canvas.height) {
		...
		drawSceneToCanvas(addInfoToExpResults, currTrialN, ctx)
	}
	...
	if (addInfoToExpResults) {
		pushTrialInfo();
		allTrials[currTrialN]['sceneDisplayStart'] = performance.now();
		if (!document.hasFocus()) {
			allTrials[currTrialN]['trialEvents'].push({'event':'sceneDisplay','details':'out of focus','performance time':performance.now(),'date time':Date.now()});
		}	
		if (document.visibilityState == 'hidden') {
			allTrials[currTrialN]['trialEvents'].push({'event':'sceneDisplay','details':'hidden','performance time':performance.now(),'date time':Date.now()});
		}
		sceneDisplayTimer = setTimeout(function() {drawISIDisplay(true)},sceneDisplayDuration);
	}
}

var ISIText;
var drawISIDisplay = function(addInfoToExpResults) {
	trialVariables = expVariables[currTrialN];
	ISI = trialVariables['ISI'];
	clearTimeout(sceneDisplayTimer);
	if (addInfoToExpResults) {
		allTrials[currTrialN]['ISIDisplayStart'] = performance.now();
	}
	if (ISI > 0) {
		...
		if (ISIText == null) {
			ISIText = new textBox(ISIText, "...", 40, BLACK);
		} else {
			ISIText.removeText();
		}
		ISIText.showText(0, canvas.height/2);
		if (addInfoToExpResults) {
			if (!document.hasFocus()) {
				allTrials[currTrialN]['trialEvents'].push({'event':'ISIDisplay','details':'out of focus','performance time':performance.now(),'date time':Date.now()});
			}
			if (document.visibilityState == 'hidden') {
				allTrials[currTrialN]['trialEvents'].push({'event':'ISIDisplay','details':'hidden','performance time':performance.now(),'date time':Date.now()});
			}
			ISITimer = setTimeout(function() {
				ISIText.removeText();
				drawChoiceDisplay(true);
			},ISI*1000);
		}
	} else {
		drawChoiceDisplay(true);
...
var drawChoiceDisplay = function(addInfoToExpResults) {
	trialVariables = expVariables[currTrialN];
	...
	if (trialCanvas.width == canvas.width && trialCanvas.height == canvas.height) {
		...
		drawChoiceStimuliToCanvas(addInfoToExpResults, currTrialN, ctx)
	}

	if (addInfoToExpResults) {
		t1 = performance.now();
		t1_UNIX = Date.now();
		allTrials[currTrialN]['choiceDisplayStart'] = t1;
		allTrials[currTrialN]['choiceDisplayStart_UNIX'] = t1_UNIX;
		if (!document.hasFocus()) {
			allTrials[currTrialN]['trialEvents'].push({'event':'choiceDisplay','details':'out of focus','performance time':performance.now(),'date time':Date.now()});
		}
		if (document.visibilityState == 'hidden') {
			allTrials[currTrialN]['trialEvents'].push({'event':'choiceDisplay','details':'hidden','performance time':performance.now(),'date time':Date.now()});
		}
...
var pushTrialInfo = function() {
	var currTrial = new trial({'trialN':currTrialN, 'stimuli':stimuli[currTrialN], 'trialEvents':[]});
	...
	t1_UNIX = Date.now();
	allTrials[currTrialN].ISI = expVariables[currTrialN]['ISI'];
...
var checkKeyPress = function(e) {
	...
	if (onChoiceDisplay == true && inConfirmation == false && keyIsDown == false && timePressed > t1 && !svg.contains(blankScreenCover)) { 
		keyIsDown = true;
		t2 = timePressed;
...
var nextTrial = function() {
	...
	allTrials[currTrialN].trialDuration = t2 - allTrials[currTrialN].trialStartTime;
	allTrials[currTrialN].trialEndTime_UNIX = t2_UNIX;
	allTrials[currTrialN].trialDuration_UNIX = t2_UNIX - allTrials[currTrialN].trialStartTime_UNIX;
	confirmationBox.removeBox()
...
var resizeWindow = function() {
	...
	if (onISIDisplay) {
		drawISIDisplay(false);
	}
	if (onChoiceDisplay) {
		drawChoiceDisplay(false);
	}
}
...
window.onresize = function() {
	...
	allTrials[currTrialN]['trialEvents'].push({'event':'onresize','details':'resize','performance time':performance.now(),'date time':Date.now()});
	resizeWindow();
}
